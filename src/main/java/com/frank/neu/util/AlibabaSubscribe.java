...
public class AlibabaSubscribe {
	...
	private static AlibabaSubscribe instance;
	
	private AlibabaSubscribe(){
		totalCounts = new AtomicInteger(0);
		merchants = Lists.newArrayList();
		map = Maps.newConcurrentMap();
	}
	
	public synchronized static AlibabaSubscribe getInstance(){
		if(null == instance){
			instance = new AlibabaSubscribe(); 
		}
		return instance;
	}
